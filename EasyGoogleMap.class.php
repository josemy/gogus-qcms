...
class EasyGoogleMap {
	...
	function SetMarkerImage($iconimage) {
		$this->iImage = $iconimage;
	}
	/**
	...
	function InitJs() {
		...
		if ($is_error) { 
			...
			$imagine = "resize_image.php?image=".$this->iImage."&new_width=31&new_height=35";
			...
			//$ret .= "	icon.infoWindowAnchor = new GPoint(100,100);\n";
			...
			for ($i=$cnt_add-1; $i>=0; $i--) {
				...
				# Comentar las dos siguientes lineas para ocultar el globo inicial.
				//if ($i===0)
				
				//$ret .= "		  marker.openInfoWindowHtml(address_$i.infowindowtext,{pixelOffset: new GSize(0,50)});\n";
				
				$ret .= "		  gmarkers[$i] = marker;\n";

				$ret .= "		}\n";
				$ret .= "		else {\n";
				//Si el mapa es incapaz de encontrar lo que se le pide se centra en madrid
				$ret .= "		  map.setCenter(new GLatLng(40.4167088,-3.5812692), {$this->mMapZoom});\n";
				$ret .= "		}\n";
				$ret .= "	  }\n";
