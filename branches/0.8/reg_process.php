...
$provin=$_POST['prov'];
$cpost=$_POST['cp'];
$direccion=$direccion." ".$cpost." ".$provin;
$telf=$_POST['telf'];
$paypal=$_POST['paypal'];
...
//Logins prohibidos
	...
	printf("<script>document.location.href='reg_form.php?item=El codigo Antispam es incorrecto&error=antispam'</script>;");
	die("");
	}
  ...
  if($username && $email && $password && $confirmpassword){
   if($password == $confirmpassword){
    ...
    $secondresult=mysql_query($secondsql);
	  ...
	  printf("<script>document.location.href='reg_form.php?item=El email (".$email.") ya esta registrado por otro usuario.'</script>;");
      die();
      }
            if (mysql_num_rows($result) > 0){
	      printf("<script>document.location.href='reg_form.php?item=El usuario (".$username.") ya esta registrado, escoja otro.'</script>;");
      die();
      }
...
//Crear directorio del usuario con permisos 777
  ...
  printf("<script>document.location.href='index.php?item=Ok.'</script>;");
  	...
  	printf("<script>document.location.href='reg_form.php?error=nopass'</script>;");
	die("");
  die();
  }
  }else{
  printf("<script>document.location.href='reg_form.php?item=Ko'</script>;");
