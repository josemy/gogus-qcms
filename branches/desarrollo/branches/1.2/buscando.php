...
//Solucion Issue 6
	//$ver_todo=mysql_query("SELECT * FROM markers WHERE (ROUND(lat,4) LIKE '$new_lat%%' AND ROUND(lng,4) LIKE '$new_long%%');");
		...
		if(!$pat){
$ver_todo=mysql_query("SELECT *,(acos(sin(radians(lat)) * sin(radians(".$pre_lat.")) + 
cos(radians(lat)) * cos(radians(".$pre_lat.")) * cos(radians(lng) - radians(".$pre_lon."))) * 6378) as 'AA' FROM `markers` WHERE (acos(sin(radians(lat)) * sin(radians(".$pre_lat.")) + cos(radians(lat)) * cos(radians(".$pre_lat.")) * cos(radians(lng) - radians(".$pre_lon."))) * 6378) < '3000' ORDER BY `AA` ASC LIMIT $com,$art;");
			...
			//
			$ver_todo=mysql_query("SELECT *,priority,(acos(sin(radians(lat)) * sin(radians(".$pre_lat.")) + 
cos(radians(lat)) * cos(radians(".$pre_lat.")) * cos(radians(lng) - radians(".$pre_lon."))) * 6378) as 'AA' FROM `markers` WHERE (acos(sin(radians(lat)) * sin(radians(".$pre_lat.")) + cos(radians(lat)) * cos(radians(".$pre_lat.")) * cos(radians(lng) - radians(".$pre_lon."))) * 6378) < '3000' ORDER BY `AA`,`priority` DESC LIMIT $com,$art;");
			}
}else{
	...
	//$ver_todo=mysql_query("SELECT * FROM markers WHERE  (name LIKE '%%$sol%%$pat%%') OR (address LIKE '%%$pat%%') OR (name LIKE '%%$pat%%') OR (name LIKE '%%$sol%%') ORDER BY priority DESC LIMIT $com,$art;");
			if($pre_city){
			$ver_todo=mysql_query("SELECT * FROM markers WHERE city = '$pre_city' ORDER BY priority DESC LIMIT $com,$art;");
			//	$ver_todo=mysql_query("SELECT *,(acos(sin(radians(lat)) * sin(radians(".$pre_lat.")) + cos(radians(lat)) * cos(radians(".$pre_lat.")) * cos(radians(lng) - radians(".$pre_lon."))) * 6378) as 'AA' FROM `markers` WHERE (acos(sin(radians(lat)) * sin(radians(".$pre_lat.")) + cos(radians(lat)) * cos(radians(".$pre_lat.")) * cos(radians(lng) - radians(".$pre_lon."))) * 6378) < '3000' ORDER BY `AA` ASC LIMIT $com,$art;");
...
//Guardar la busqueda para las estadisticas de busquedas recientes.
	...
	mysql_query("INSERT INTO busquedas (id,busqueda,num_result,ip,referer,timestamp,username) VALUES ('','$rpat','$cuantos_re','$iip','$referer','$times','$username')");
//
		...
		if($mode=="debug"){
			...
			echo("paginas: ".$num_p);
			echo(" Coordenadas de la localidad: ".$pre_lat."".$pre_lon." ");
		}
		//
