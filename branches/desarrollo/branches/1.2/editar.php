...
$resulta=mysql_query("UPDATE `markers` SET `descripcion` = '$desc', `reserva_onl` = 'si' , `desc_long` = '$ldesc' , `precio_medio` = '$precio' , `web` = '$web' , `tipo_cocina` = '$tipo_co' , `tags` = '$etiq' , `phone` = '$phone' , `image` = '$image' , `address` = '$address' WHERE `markers`.`id` ='$idd' LIMIT 1 ;");
   
if (!$resulta) {
  die('Invalid query: ' . mysql_error());
}

print("<script>document.location.href='sitio/".$titulo_f."'</script>");
?>