<div style="position:relative; padding-left:2px;">
  ...
  <table width="100%"  border="0" cellpadding="4" cellspacing="4" class="roudlite_g_b" style=" left:2px;" >
    <tr>
      <td class="roudlite_g_a" ><a class="invitado_home2"><strong>Cerca de ti</strong></a></td>
    </tr>
    <tr>
	...
	//Nueva query:
	$ver_todo=mysql_query("SELECT *,(acos(sin(radians(lat)) * sin(radians(".$location["Latitude"].")) + 
cos(radians(lat)) * cos(radians(".$location["Latitude"].")) * cos(radians(lng) - radians(".$location["Longitude"]."))) * 6378) as 'AA' FROM `markers` WHERE (acos(sin(radians(lat)) * sin(radians(".$location["Latitude"].")) + cos(radians(lat)) * cos(radians(".$location["Latitude"].")) * cos(radians(lng) - radians(".$location["Longitude"]."))) * 6378) < '3000' ORDER BY `AA` ASC LIMIT 0,10;");
